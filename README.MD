# 26E2 - INFNET - Trabalho PD modelos preditivos utilizando KNIME

## Author: Alan C. Echer

## Este trabalho está disponível no github: https://github.com/echer/26E2-infnet-trabalho-pd-modelos-preditivos.git

## 1. Etapa 1 – Coleta e Preparação dos Dados

### 1.1. Coleta dos dados

![dataset-kaggle](assets/dataset-kaggle.png)

Para este trabalho utilizarei a base de dados do kaggle: https://www.kaggle.com/datasets/sunil123kumar/social-media-impact-on-mental-health

A base foi escolhida por abordar um problema atual e de grande relevância social: a relação entre o uso de redes sociais e indicadores de saúde mental em adolescentes. Além disso, apresenta variáveis numéricas e categóricas, sendo adequada para aplicação de diferentes algoritmos de classificação estudados na disciplina.

A base possui 1200 registros válidos e nenhuma variável apresenta valores ausentes, ela contém dados da utilização de redes sociais por adolescentes, seus hábitos, performance escolar e indicadores mentais.

A base reúne informações sobre:

* idade;
* gênero;
* tempo diário de uso das redes sociais;
* plataforma utilizada;
* horas de sono;
* tempo de tela antes de dormir;
* desempenho escolar;
* atividade física;
* interação social;
* níveis de estresse;
* ansiedade;
* dependência de redes sociais;
* indicador de depressão.

### 1.2. Preparação dos dados

#### 1.2.1. Valores ausentes

Foi identificado que não existem valores faltantes em nenhuma das 13 variáveis.

Portanto, não foi necessária nenhuma técnica de imputação.

#### 1.2.2. Tipos de dados

Foi realizada a identificação das variáveis numéricas e categóricas.

As variáveis categóricas serão posteriormente convertidas utilizando One-Hot Encoding, permitindo sua utilização pelos algoritmos de Machine Learning.

#### 1.2.3. Classe alvo

Inicialmente a variável alvo considerada foi Depression Label.

Entretanto, observou-se um forte desbalanceamento entre as classes, com aproximadamente 97,4% dos registros pertencentes à classe "não deprimido" e apenas 2,6% à classe "deprimido".

Nas etapas de modelagem serão avaliadas técnicas para lidar com esse desbalanceamento, como SMOTE ou Oversampling, conforme discutido na disciplina.

## 2. Etapa 2 – Análise exploratória (EDA)

Realizou-se uma análise exploratória dos dados utilizando os componentes Statistics e Data Explorer do KNIME. O objetivo foi compreender a distribuição das variáveis, identificar possíveis inconsistências e conhecer as características da base antes da construção dos modelos preditivos.

* As variáveis numéricas apresentam distribuição relativamente homogênea.
* As variáveis categóricas possuem poucas categorias e são adequadas para codificação por One-Hot Encoding.
* A variável Gender apresenta distribuição equilibrada entre masculino e feminino.
* A variável Platform Usage possui três categorias (Instagram, TikTok e Both).
* A variável Depression Label apresenta forte desbalanceamento.

### 2.1. Column: Age - Idade do estudante (Numérica discreta)

As idades variam de 13 a 19 anos, sua média é de 15.9 e o desvio padrão é de 2.02. Os dados estão bem distribuidos.

![column_age.png](assets/column_age.png)

### 2.2. Coluna: Gender - Gênero do estudante (Categórica nominal)

O genero só possui dois tipos male e female e estão muito bem distribuidos em 51% e 49%

![column_gender](./assets/column_gender.png)

### 2.3. Column: Daily Social Media Hours - Horas gastas por dia em redes sociais (Numérica contínua)

As horas gastas por dias em redes sociais estão também bem distribuidas variando de 1 a 8 horas, média de 4.54 e desvio padrão de 2.03.

![column_daily_social_media_hours](./assets/column_daily_social_media_hours.png)

### 2.4. Column: Platform Usage - Uso de plataforma sociais (Categórica nominal)

A variável possui três categorias (Instagram, TikTok e Both), sendo que "Both" representa estudantes que utilizam ambas as plataformas. Como trata-se de uma variável categórica nominal, posteriormente será transformada por meio de One-Hot Encoding para utilização pelos algoritmos de classificação.

![column_platform_usage](./assets/column_platform_usage.png)

### 2.5. Column: Sleep Hours - Horas de sono (Numérica contínua)

A coluna horas de sono varia de 4 a 9 e tem dados bem distribuidos com uma média de 6.45 e um desvio padrao de 1.44.

![sleep_hours](./assets/column_sleep_hours.png)

### 2.6. Column: Screen Time Before Sleep - Tempo de tela antes de dormir (Numérica contínua)

A coluna tempo de tela antes de dormir possui dados variando de 0.5 até 2.4 horas com uma média de 1.74 e uma variação de 0.72, os dados são bem distribuidos.

![column_screen_time_before_sleep](./assets/column_screen_time_before_sleep.png)

### 2.7. Column: Academic Performance - Performance escolar (Numérica contínua)

A coluna performance escolar tem dados bem distribuidos variando de 2 a 4 com média de 2.99 e variação de 0.58.

![column_academic_performance](./assets/column_academic_performance.png)

### 2.8. Column: Physical Activity - Atividade fisica (Numérica contínua)

A coluna apresenta maior concentração em determinados níveis de atividade física, porém sem indícios de inconsistências, com alguns picos com valores variando de 0 a 2, média de 1.01 e desvio de 0.58.

![column_physical_activity](./assets/column_physical_activity.png)

### 2.9. Column: Social Interaction Level - Nível de interação social (Categórica ordinal)

A coluna nivel de interação social possui 3 valores: high 31%, medium 35%, low 35%, até que bem distribuidos.

![column_social_interaction_level](./assets/column_social_interaction_level.png)

### 2.10. Column: Stress Level - Nível de estresse (Numérica discreta)

A coluna nivel de estresse possui valores até que bem distribuidos variando de 1 a 10, com média de 5.45 e desvio de 2.9.

![column_stress_level](./assets/column_stress_level.png)

### 2.11. Column: Anxiety Level - Nível de ansiedade (Numérica discreta)

A coluna nivel de ansiedade também de valores bem distribuidos variando de 1 a 10, com média de 5.65 e desvio de 2.86.

![column_anxiety_level](./assets/column_anxiety_level.png)

### 2.12. Column: Addiction Level - Nível de dependência (Numérica discreta)

A coluna nível de dependência também tem dados bem distribuidos variando de 1 a 10, com média de 5.57 e desvio de 2.83.

![column_addiction_level](./assets/column_addiction_level.png)

### 2.13. Column: Depression Label - Rótulo de depressão (Categórica binária)

A coluna depression label é a coluna explicativa do nosso dataset e ela define se o estudante possui depressão ou não, se 0 = não tem depressão, se 1 = tem depressão.

![column_depression_label](./assets/column_depression_label.png)

As estatisticas completas das colunas e o histograma podem ser conferidas no knime:

![knime-statistics](./assets/knime-statistics.png)

### 2.14. Desbalanceamento da classe alvo

A variável alvo apresenta forte desbalanceamento, com aproximadamente 97,4% (1169) dos registros pertencentes à classe "não deprimido" e apenas 2,6% (31) à classe "deprimido". Esse desbalanceamento pode prejudicar o treinamento dos modelos, favorecendo a classe majoritária. Para minimizar esse problema poderão ser utilizadas técnicas como SMOTE ou Oversampling durante o pré-processamento.

![knime-depression-barchart](./assets/knime-depression-barchart.png)

### 2.15. Matrix de correlação

A matriz de correlação demonstra que não existem correlações lineares fortes entre as variáveis numéricas. Todos os coeficientes encontrados são muito próximos de zero, variando aproximadamente entre -0,064 e 0,076.

As maiores correlações observadas foram:

* +0,076 - Age × Screen Time Before Sleep
* -0,064 - Academic Performance × Anxiety Level
* -0,055 - Sleep Hours × Addiction Level

Mesmo essas relações são consideradas muito fracas, indicando que nenhuma variável, isoladamente, apresenta forte relação linear com outra.

![knime-correlation-matrix](./assets/knime-correlation-matrix.png)

De forma geral, a análise exploratória mostrou que a base apresenta boa qualidade, ausência de valores faltantes e variáveis bem distribuídas. O principal desafio identificado foi o forte desbalanceamento da variável alvo, fator que motivou a utilização de técnicas específicas durante a etapa de modelagem.

## 3. Etapa 3 - Pré processamento

### 3.1. One hot encoding

Foram aplicados técnicas de one hot encoding para transformar as variáveis categóricas: gender, platform_usage e social_interaction_level em variáveis numéricas para serem utilizadas no treinamento.

![knime-one-hot-encoding](./assets/knime-one-hot-encoding.png)

A técnica transforma cada categoria em uma nova coluna, por exemplo a coluna gender foi transformada em duas colunas: male e female.

![knime-one-hot-encoding-male-female](./assets/knime-one-hot-encoding-male-female.png)

### 3.2. Normalização

Foi aplicado também uma técnica de normalização nas colunas pois estavam em escalas diferentes, evitando que atributos muito maiores influenciassem no modelo: age, daily_social_media_hours, sleep_hours, screen_time_before_sleep, academic_performance, physical_activity, stress_level, anxiety_level, addiction_level.

![knime-normalization](./assets/knime-normalization.png)

### 3.3. Separação treino / teste

Para comparar diferentes modelos de classificação de maneira justa foi utilizada a técnica de validação cruzada (Cross Validation). Nessa abordagem, a base de dados é dividida em diferentes subconjuntos (folds). Ao final das 10 iterações, as métricas de desempenho são calculadas em cada fold e a média dessas métricas representa o desempenho final do modelo. Além disso, foi utilizada a divisão estratificada, preservando a proporção entre as classes em cada partição de treinamento e teste, garantindo uma avaliação mais representativa do desempenho dos modelos.

A separação dos dados em treino é teste foi feito para garantir que não haja vazamento de dados entre os dados de treino e dados de teste, além disso foi utilizado uma estratégia de estratificação dos dados 1.200 linhas em 10 partes (folds) de 120 linhas estratificando pela coluna depression_label, garantindo que cada fold contenha aproximadamente a mesma proporção de deprimidos (3) e não deprimidos (117), esta estratégia garante que os dados consigam ser separados treinados e testados da forma mais confiável possível já que a base é bem desbalanceada tendo apenas 31 registros com depressão de 1.200.

![knime-split-train-test](./assets/knime-split-train-test.png)

## 4. Etapa 4 - Modelagem Preditiva

### 4.1. Modelo 1 Regressão Logistica

Embora possuam nomes semelhantes, a regressão linear e a regressão logística possuem objetivos diferentes. A regressão linear é utilizada para prever valores numéricos contínuos, como preço de imóveis, temperatura ou faturamento. Seu resultado pode assumir qualquer valor dentro de um intervalo contínuo. Já a regressão logística é utilizada em problemas de classificação. Em vez de prever um valor contínuo, ela estima a probabilidade de uma observação pertencer a uma determinada classe. Para isso, utiliza a função logística (sigmoide), que transforma o resultado em um valor entre 0 e 1. A partir dessa probabilidade, define-se a classe prevista, como por exemplo "com depressão" ou "sem depressão". Neste trabalho foi utilizada a regressão logística porque o objetivo é classificar os estudantes em duas classes (com depressão e sem depressão), caracterizando um problema de classificação binária.

No modelo 1 para a classe 1 = com depressão o modelo obteve uma precisao de 61.3% e um recall de 67.9%.

- ACERTOS: tivemos 19 TP (estudantes que tinham depressão e o modelo categorizou como depressão)
- ACERTOS: tivemos 1.160 TN (estudantes que não tinham depressão e o modelo categorizou como não depressão)
- ERROS: tivemos 9 FN (estudantes que tinham depressão e foram categorizados sem depressão)
- ERROS: tivemos 12 FP (estudantes que não tinham depressão e foram categorizados com depressão)

![knime-model-lr](./assets/knime-model-lr.png)

### 4.2. Modelo 2 Regressão Logistica + Data Augmentation (SMOTE)

![knime-model-lr-smote](./assets/knime-model-lr-smote.png)

No modelo 2 para a classe 1 = com depressão o modelo obteve uma precisão de 83.9% e um recall de 36.6%

* ACERTOS: tivemos 26 TP (estudantes que tinham depressão e o modelo categorizou como depressão)
* ACERTOS: tivemos 1.124 TN (estudantes que não tinham depressão e o modelo categorizou como não depressão)
* ERROS: tivemos 45 FN (estudantes que tinham depressão e foram categorizados sem depressão)
* ERROS: tivemos 5 FP (estudantes que não tinham depressão e foram categorizados com depressão)

### 4.3. Comparação Modelo 1 vs Modelo 2

Ao comparar os dois modelos, observa-se um comportamento distinto entre eles. O Modelo 1 apresentou maior Recall, sendo capaz de identificar uma quantidade maior de estudantes com depressão. Como consequência, também gerou um número maior de falsos positivos, classificando alguns estudantes sem depressão como deprimidos. Entretanto, essa abordagem reduz a quantidade de casos reais de depressão que deixam de ser identificados, característica desejável em aplicações de triagem na área da saúde.

Já o Modelo 2 apresentou uma abordagem mais conservadora. Sua maior precisão indica que, quando o modelo classifica um estudante como portador de depressão, essa previsão tende a estar correta na maioria dos casos. Em contrapartida, o baixo Recall demonstra que o modelo deixou de identificar muitos estudantes que realmente apresentavam depressão, classificando incorretamente como não deprimidos. Assim, embora produza menos falsos positivos, o Modelo 2 gera um número significativamente maior de falsos negativos.

Considerando que o objetivo deste trabalho é identificar estudantes com depressão, o Modelo 1 mostrou-se mais adequado, pois prioriza a identificação da classe de interesse, mesmo ao custo de aumentar a quantidade de falsos positivos.

Vamos agora fazer outros modelos.

### 4.4. Modelo 3 Decision Tree

No modelo 3 para a classe 1 = com depressão o modelo obteve uma precisão de 93.5% e um recall de 100%

* ACERTOS: tivemos 29 TP (estudantes que tinham depressão e o modelo categorizou como depressão)
* ACERTOS: tivemos 1.169 TN (estudantes que não tinham depressão e o modelo categorizou como não depressão)
* ERROS: tivemos 0 FN (estudantes que tinham depressão e foram categorizados sem depressão)
* ERROS: tivemos 2 FP (estudantes que não tinham depressão e foram categorizados com depressão)

O modelo até o momento está sendo o melhor ele conseguiu categorizar 100% dos estudantes que tinham depressão como depressão, um recall excelente para a área da saude, e para os que não tinham depressão ele categorizou apenas 2 resultado incorretamente como depressão uma taxa baixissima de falsos positivos.

![knime-model-decision-tree](./assets/knime-model-decision-tree.png)

### 4.5. Modelo 4 Random Forest

No modelo 4 para a classe 1 = com depressão o modelo obteve uma precisão de 90.3% e um recall de 100%

* ACERTOS: tivemos 28 TP (estudantes que tinham depressão e o modelo categorizou como depressão)
* ACERTOS: tivemos 1.169 TN (estudantes que não tinham depressão e o modelo categorizou como não depressão)
* ERROS: tivemos 0 FN (estudantes que tinham depressão e foram categorizados sem depressão)
* ERROS: tivemos 3 FP (estudantes que não tinham depressão e foram categorizados com depressão)

O modelo 4 assim como o 3 obteve um excelente resultado de recall e de precisão com uma diferença de apenas 1 TP e 1 FP entre eles, pode ser dizer que os modelos tiveram resultados bem semelhantes, o modelo 4 pelo seu método de classificação e generalização e por ser menos sucetivel a overfiting seria uma escolha um pouco mais segura do que o decision tree até o momento.

![knime-model-random-forest](./assets/knime-model-random-forest.png)

### 4.6. Modelo 5 Gradient Boosted Trees (extra)

No modelo 5 para a classe 1 = com depressão o modelo obteve uma precisão de 96.8% e um recall de 100%

* ACERTOS: tivemos 30 TP (estudantes que tinham depressão e o modelo categorizou como depressão)
* ACERTOS: tivemos 1.169 TN (estudantes que não tinham depressão e o modelo categorizou como não depressão)
* ERROS: tivemos 0 FN (estudantes que tinham depressão e foram categorizados sem depressão)
* ERROS: tivemos 1 FP (estudantes que não tinham depressão e foram categorizados com depressão)

Até o momento o modelo 5 tem o melhor resultado de performance acertando todos os que tem depressão e categorizando errado apenas 1 estudante que não tinha depressão como depressão.

![knime-model-gradient-boosted-trees](./assets/knime-model-gradient-boosted-trees.png)

### 4.7. Modelo 6 SVM (extra)

Também foram treinados outro modelo utilizando SVM porém não obtive muito sucesso e demandaria um pouco mais de aprofudamento no rune de hyperparametros e experimentação.

![knime-model-svm](./assets/knime-model-svm.png)

### 4.8. Modelo 7 Rede Neural Rasa

No modelo 7 para a classe 1 = com depressão o modelo obteve uma precisão de 80.6% e um recall de 69.4%

* ACERTOS: tivemos 25 TP (estudantes que tinham depressão e o modelo categorizou como depressão)
* ACERTOS: tivemos 1.158 TN (estudantes que não tinham depressão e o modelo categorizou como não depressão)
* ERROS: tivemos 11 FN (estudantes que tinham depressão e foram categorizados sem depressão)
* ERROS: tivemos 6 FP (estudantes que não tinham depressão e foram categorizados com depressão)

O modelo de rede neural rasa foi feito utilizando 1 hidden layer e 50 neuronios por layer, durante 1.000 interações. O modelo não performou muito bem.

![knime-model-neural-network-shallow](./assets/knime-model-neural-network-shallow.png)

### 4.9. Modelo 8 Rede Neural Profunda

O Deep Learning é uma evolução das redes neurais artificiais, utilizando várias camadas ocultas para aprender padrões mais complexos nos dados. Esse tipo de modelo costuma apresentar um desempenho muito bom em problemas como reconhecimento de imagens, voz e texto, principalmente quando há uma grande quantidade de dados disponível. Porém, também exige maior capacidade computacional e mais tempo para treinamento. Neste trabalho foi utilizada uma rede neural rasa, conforme solicitado pelo professor, enquanto o Deep Learning foi estudado apenas de forma conceitual para compreender sua aplicação e suas diferenças em relação às redes neurais tradicionais.

### 4.10. Comparação dos Modelos


| Modelo                             | Acurácia | Precisão |   Recall   | F1-Score | Observação                                                                                                                                        |
| ---------------------------------- | :-------: | :-------: | :--------: | :-------: | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Regressão Logística**          |   98,3%   |   61,3%   |   67,9%   |   64,4%   | Modelo simples e interpretável. <br />Apresentou recall razoável, porém baixa<br /> precisão para identificar estudantes <br />com depressão.  |
| **Regressão Logística + SMOTE**  |   95,8%   |   83,9%   |   36,6%   |   51,0%   | O balanceamento aumentou a precisão,<br /> porém reduziu significativamente o recall,<br /> deixando de identificar muitos casos<br /> positivos. |
| **Árvore de Decisão**            |   99,8%   |   93,5%   |   100,0%   |   96,6%   | Excelente desempenho, identificando todos<br /> os casos positivos. Fácil interpretação e<br /> ótimo equilíbrio entre métricas.              |
| **Random Forest**                  |   99,8%   |   90,3%   |   100,0%   |   94,9%   | Resultado muito semelhante ao da Árvore <br />de Decisão, porém mais robusto contra <br />overfitting.                                           |
| **Gradient Boosted Trees (extra)** | **99,9%** | **96,8%** | **100,0%** | **98,4%** | Melhor desempenho entre todos os modelos <br />avaliados, apresentando a maior precisão<br /> sem perder recall.                                   |
| **SVM (extra)**                    |   98,0%   |   79,4%   |   72,2%   |   75,6%   | Obteve desempenho intermediário. <br />Provavelmente poderia melhorar com ajuste<br /> de hiperparâmetros.                                        |
| **Rede Neural Rasa**               |   98,6%   |   80,6%   |   69,4%   |   74,6%   | Não superou os modelos baseados em <br />árvores para esta base de dados.                                                                         |

Considerando apenas os modelos exigidos pelo trabalho, a Árvore de Decisão seria a escolha mais indicada por combinar excelente desempenho com simplicidade e facilidade de interpretação. Entretanto, considerando todos os modelos treinados, incluindo os experimentos extras, o Gradient Boosted Trees apresentou o melhor desempenho geral, alcançando os maiores valores de acurácia, precisão, recall e F1-score. Dessa forma, ele seria o modelo recomendado para uma eventual implantação em um ambiente real, enquanto a Árvore de Decisão permanece como a melhor alternativa entre os modelos obrigatórios da disciplina.
